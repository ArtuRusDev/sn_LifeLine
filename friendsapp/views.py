...
def send_friend_request(request, pk):
    ...
    duplicate = FriendRequests.objects.filter(initiator=Person.objects.get(pk=pk), target=request.user) | FriendRequests.objects.filter(initiator=request.user, target=Person.objects.get(pk=pk))
